...
class VCOCOeval(object):
    ...
    # return:
    # agents = n(det num) * [box-coordinates, 26 * score]
    def _collect_detections_for_image(self, dets, image_id):
        ...
        # one det is one person
        for det in dets:
            if det['image_id'] == image_id:
                # bbox coordinates + action classes score
                this_agent = np.zeros((1, 4 + self.num_actions), dtype=np.float32)
                # 26 * (bbox coordinates + score) * 2(role)
    ...
    def _do_role_eval(self, vcocodb, detections_file, ovr_thresh=0.5, eval_type='scenario_1'):
        ...
        # don't care about human-cls score

    # only use human-action-cls score
                ...
                for j in idx:
                    ...
                    if is_true_action and (ovmax >= ovr_thresh):
                        if covered[jmax]:
                            ...
                            # only highest score action is true positive, others is fp
                            fp[aid].append(0)
                            tp[aid].append(1)
                            covered[jmax] = True
                    else:
